...
Before deploying, apply migration `0006_queue_first_ingestion.sql`. It terminalizes legacy `pending` jobs and only lease-less or expired `sending` jobs that relied on the removed repair publisher; actively leased consumers are left untouched. Already queued and delayed-retry jobs keep their Queue-owned lifecycle and their previous untagged messages remain decodable. Tagged messages must match the supported command version and cannot fall through to legacy decoding. The migration also adds the atomic fan-out marker and durable ingestion-failure ledger. Terminal counts are visible in administrator status, with reasons retained in D1.
