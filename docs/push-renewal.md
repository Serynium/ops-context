...
- After an upgrade, the next authenticated PWA launch enrolls any existing local browser subscription that does not yet have an IndexedDB credential. Disabled rows cannot be enabled directly; they must be re-enrolled from their installation so the browser receives a new credential.
