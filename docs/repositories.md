...
`D1RepositoriesLive` builds all repository implementations from one D1 binding. The application Layers capture only the individual repository and infrastructure services they use, so no broad Effect context is retained or re-provided.
